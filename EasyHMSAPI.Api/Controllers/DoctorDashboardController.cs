    ...
    public class DoctorDashboardController : ControllerBase
    {
        ...
        public async Task<ActionResult<GetDoctorDashboardAnalysisResponseModel>> GetDoctorDashboardAnalysis(Guid hospitalId, Guid doctorId)
        {
            ...
            try
            {
                ...
                GetDoctorDashboardAnalysisRequestModel request = new()
                {
                    HospitalId = hospitalId,
                    DoctorId = doctorId
                };
                var response = await _mediator.Send(request);
                _logger.LogInformation("GetDoctorDashboardAnalysis ended for hospitalId: {HospitalId}, doctorId: {DoctorId}", hospitalId, doctorId);
                return Ok(response);
            }
            catch (Exception ex)
            {
                _logger.LogError("Error in GetDoctorDashboardAnalysis for hospitalId: {HospitalId}, doctorId: {DoctorId}. Error: {Error}", hospitalId, doctorId, ex);
                return StatusCode(500, new { Message = "An error occurred while retrieving dashboard analysis", Error = ex.Message });
            }
        }
    }
}
